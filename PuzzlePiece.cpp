...
const float PUZZLE_PI = 3.141592654f;
float CPuzzlePiece::rotationAngles[] = { 0.0f, -PUZZLE_PI / 2.0f, -PUZZLE_PI, PUZZLE_PI / 2.0f };

ID3D11Buffer* CPuzzlePiece::vertexBuffer = nullptr;
...
void CPuzzlePiece::Add(int index, uint8_t* pImage, float scale, int palette[], int numberOfImages, float left, float top, int positionOffset)
{
	...
	if (dx.Map(pTexture, 0, D3D11_MAP_WRITE_DISCARD, 0, &subRes) == 0)
	{
		int inPtr = 4;
...
void CPuzzlePiece::Render()
{
	...
	if (vertexBuffer != nullptr)
	{
		...
		for (auto pPiece : ZImages)
		{
			...
			// Combine scaling, rotation, and translation using our Math abstractions
			float16 wm = Math::Scaling(w, h, 1.0f) * Math::RotationZ(rotationAngles[pPiece->Orientation]) * Math::Translation(x, -y, 0.0f);
