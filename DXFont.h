...
public:
    CDXFont();
    CDXFont(uint8_t* pFont, uint32_t size);
    virtual ~CDXFont();

    void Init(uint8_t* pFont, uint32_t size);
    ...
    Size GetSize(const char* text, float maxw);

    std::list<PointUV> Create(const char* text, bool texFont);
