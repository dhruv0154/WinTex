...
{
    uint32_t s1 = 0;
    uint8_t* p1 = GetResource(IDB_BUBBLE, "PNG", &s1);
...
void CDXDialogueOption::SetClick(void(*Clicked)(void* data))
{
    _clicked = Clicked;
}
...
void CDXDialogueOption::SetText(char* text, Size sz, float x)
{
    ...
    Rect rc;
    rc.Left = 0;
    rc.Right = static_cast<int>(sz.Width);
    rc.Top = 0;
    rc.Bottom = static_cast<int>(sz.Height);
...
void CDXDialogueOption::Render()
{
    ...
    // Draw text
    int mask = (_mouseOver && _enabled) ? -1 : static_cast<int>(0x80ffffff);
